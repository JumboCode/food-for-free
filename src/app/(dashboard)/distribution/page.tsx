...
import { Search, Loader2, Download, ChevronRight } from 'lucide-react';
...
function DistributionContent() {
    ...
    const handleRowClick = async (record: DeliveryRecord) => {
        ...
        const params = new URLSearchParams({ date: dateStr, org: record.organizationName });
        try {
            ...
            setPopupData({
                date: new Date(detail.date + 'T00:00:00').toLocaleDateString('en-US', {
                    month: 'numeric',
                    day: 'numeric',
                    year: 'numeric',
                }),
                            ...
                            <table className="w-full border-collapse" style={{ minWidth: 720 }}>
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-200">
                                        ...
                                        <th className="w-8" aria-label="View details" />
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading ? (
                    ...
                    {!loading && data.length > 0 && (
                        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                            <p className="text-sm text-gray-500">
                                Showing {startIdx + 1}-
                                {Math.min(startIdx + ROWS_PER_PAGE, data.length)} of {data.length}
                            </p>
